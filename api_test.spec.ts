...
describe("PetApi", () => {
  ...
  test("getPetById", () => {
    ...
    .uponReceiving(`A request for for pet with id ${petId}`)
    .withRequest({
      method: "GET",
      path: `/pet/${petId}`,
    })
    .willRespondWith({
      status: 200,
      body: { name: like("doggie"), id:petId },
    });

    return provider.executeTest((mockserver) => {
      instance = new api.PetApi(config, mockserver.url)
      return instance.getPetById(petId, {}).then((result) => {
         expect(result).toEqual({name: "doggie", id: petId});
      });
    });
